...
<div class="tab-pane fade show active" id="product" role="tabpanel">
    ...
    <form action="/action_ubah_produk/{{$products->id}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method("PUT")
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="categoryid" placeholder="Nama Produk" name="category_id" required>
                ...
                <option>Pilih Kaetgori Produk</option>
                @foreach ($category as $cate )
                    <option value="{{$cate->id}}" {{$cate->id == $products->category_id ? "selected" : ''}} >{{$cate->name_category}}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
